...
std::vector<std::string> Database::load_gen_names(const std::string& table_name) const
{
    ...
    Statement stmt(*this, "SELECT gen_name FROM " + table_name + " ORDER BY gen_id;");
    while (stmt.step() == SQLITE_ROW)
...
void Database::save_generators(const std::string& table_name, const std::vector<std::string>& gen_names, const std::vector<alg::Deg>& gen_degs, const alg::Poly1d& gen_reprs) const
{
    Statement stmt_update_generators(*this, "INSERT INTO " + table_name + " (gen_id, gen_name, repr, s, t, v) VALUES (?1, ?2, ?3, ?4, ?5, ?6);");
...
{
    db.sqlite3_prepare_v100(sql, &stmt_);
}
