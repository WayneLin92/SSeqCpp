...
std::map<alg::Deg, DgaBasis1> load_dga_basis(const Database& db, const std::string& table_name, int r, int t_max)
{
    ...
    while (stmt.step() == SQLITE_ROW) {
        ...
        basis[d].basis.push_back(DbStr2Mon(stmt.column_str(0)));
        basis[d].diffs.emplace_back();
        mon_diffs[d].push_back({ DbStr2Array(stmt.column_str(1)) });
    }
...
std::map<alg::Deg, DgaBasis1> load_basis_X(const Database& db, const std::string& table_name, int t_max, int r)
{
    ...
    while (stmt.step() == SQLITE_ROW) {
        ...
        basis[d].basis.push_back(DbStr2Mon(stmt.column_str(0)));
        basis[d].diffs.push_back(DbStr2Poly(stmt.column_str(1)));
    }
...
void load_y(const Database& db, const std::string& table_name, alg::Poly1d& y, alg::array& t_y)
{
    ...
    while (stmt.step() == SQLITE_ROW) {
        y.push_back(DbStr2Poly(stmt.column_str(0)));
...
alg::Poly d_inv(const alg::Poly& poly, const std::vector<alg::Deg>& gen_degs, const alg::Poly1d& diffs, const alg::Groebner& gb, const std::map<alg::Deg, DgaBasis1>& basis_A,
                const std::map<alg::Deg, DgaBasis1>& basis_X)
{
    if (poly.empty())
        return {};
