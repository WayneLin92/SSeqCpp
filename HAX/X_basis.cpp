...
std::map<alg::Deg, DgaBasis1> get_basis_X(const std::vector<alg::Deg>& gen_degs, const alg::Poly1d& gen_diffs, int start_x, int num_x, int t_max)
{
    std::map<alg::Deg, DgaBasis1> result;
    result[alg::Deg{ 0, 0, 0 }].basis.push_back({});
    result[alg::Deg{ 0, 0, 0 }].diffs.push_back({});

    for (int t = 1; t <= t_max; ++t) {
        std::map<alg::Deg, DgaBasis1> basis_new;
        ...
        for (int gen_id = start_x + num_x - 1; gen_id >= start_x; --gen_id) { /* 58 is the gen_id of b1 */
            ...
            if (t1 >= 0) {
                auto p1 = result.lower_bound(alg::Deg{ 0, t1, 0 });
                auto p2 = result.lower_bound(alg::Deg{ 0, t1 + 1, 0 });
                for (auto p = p1; p != p2; ++p) {
                    ...
                    for (; p_m != p->second.basis.end(); ++p_m, ++p_diff) {
                        if (p_m->empty() || gen_id <= p_m->front().gen) {
                            ...
                            basis_new[alg::Deg{ s, t, v }].basis.push_back(std::move(mon));
                            alg::Poly diff = add(mul(gen_diffs[gen_id], *p_m), mul({ { gen_id, 1 } }, *p_diff));
                            basis_new[alg::Deg{ s, t, v }].diffs.push_back(std::move(diff));
                        }
                    }
                }
            }
...
void save_basis_X(Database& db, int start_x, int num_x)
{
    ...
    alg::Poly1d diffs_B = db.load_gen_diffs("B_generators");
    std::map<alg::Deg, DgaBasis1> basis_X = get_basis_X(gen_degs_B, diffs_B, start_x, num_x, t_max);

    std::string table_name = "X" + std::to_string(num_x) + "_basis";
    try {
        db.execute_cmd("DROP TABLE " + table_name + ';');
    }
    catch (MyException&) {
    }
    ...
    for (auto& [deg, basis_d] : basis_X) {
        for (size_t i = 0; i < basis_d.basis.size(); ++i) {
            ...
            stmt.bind_str(4, Mon2DbStr(basis_d.basis[i]));
