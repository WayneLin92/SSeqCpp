...
public:
	...
	void save_gb(const std::string& table_name, const alg::Poly1d& gb, const std::vector<alg::Deg>& gen_degs) const { save_gb(table_name, gb, gen_degs, 0); }
...
inline std::string array_to_str(const alg::array& a) { return array_to_str(a.begin(), a.end()); }
std::string Mon_to_str(alg::MonInd pbegin, alg::MonInd pend);
inline std::string Mon_to_str(const alg::Mon& mon) { return Mon_to_str(mon.begin(), mon.end()); }
std::string Poly_to_str(alg::Poly::const_iterator pbegin, alg::Poly::const_iterator pend);
inline std::string Poly_to_str(const alg::Poly& poly) { return Poly_to_str(poly.begin(), poly.end()); }
