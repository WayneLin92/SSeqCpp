...
public:
    ...
    void save_products(const std::string& table_prefix, int id, const GenMRes2d& gens, const Mod2d& map, const array3d& map_h)
    {
        if (f_.valid())
            f_.wait();
        f_ = std::async(std::launch::async, [this, table_prefix, id, &gens, &map, &map_h]() {
            begin_transaction();
            ...
            for (size_t s = 0; s < map.size(); ++s) {
                for (size_t i = 0; i < map[s].size(); ++i) {
                    stmt.bind_blob(1, map[s][i].data);
                    stmt.bind_blob(2, map_h[s][i]);
